...
//console.log(A.toString());
    ...
    // Choose pivot based on the median method.
    // Determine the median of three possible values A[0], A[n/2],
    // and A[n-1].  Whichever is the median swap that into the A[0]
    // position.
    var pi = 0;  // pivot index to be determined by median method
    var m = Math.round(n/2)-1;

    if (A[m] > A[0] && A[m] < A[n-1]) { pi = m; }
    else if (A[m] < A[0] && A[m] > A[n-1]) { pi = m; }
    else if (A[n-1] > A[0] && A[n-1] < A[m]) { pi = n-1; }
    else if (A[n-1] < A[0] && A[n-1] > A[m]) { pi = n-1; }
    else { pi = 0; }
